...
```{r plot-affinn-100}
affinn_sentiments |> 
  group_by(film.name,  index = linenumber %/% 100) %>% 
...
When splitting up the text into 100-line chunks, you can see which movies have the most positive and negative sentiment. The 6th movie appears to be the most positive. One chunk in the middle of the movie has the highest rating of all. The most negative would be the end of the 8th movie, but the middle of the 2nd is close. The 7th movie appears to be the most neutral. 


```{r plot-affinn-8}
affinn_sentiments |> 
  group_by(film.name,  index = linenumber %/% 12) %>% 
  summarise(sentiment = sum(value)) |> 
  ggplot(aes(index, sentiment, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
  ...
  labs(
    x = "Each chunk split up by 12 lines",
    subtitle = "AFFINN method"
  ) 
```
